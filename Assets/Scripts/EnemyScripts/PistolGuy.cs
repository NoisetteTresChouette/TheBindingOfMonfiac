...
public class PistolGuy : Enemy
{
    ...
    private void Awake()
    {
        ...
        _shootingRange = GetComponent<PistolGuyShoot>().GetRange();
    ...
    private void Update()
    {
        if (_detectionZone.IsAlert())
        {
            ...
            if (Vector2.Distance(transform.position, _detectionZone.GetTarget().position) > _shootingRange)
            {
